import { mean, zip, chunk } from 'lodash-es';
...
const SoundWaveSVG = ({ soundData }) => {
  ...
  React.useEffect(async () => {
    const { AudioContext } = await import(/* webpackPrefetch: true */ 'standardized-audio-context');
    ...
    // 右の音声データの絶対値を取る
    const rightData = buffer.getChannelData(1).map(buffer.getChannelData(1), Math.abs);
